...
def acquire_and_display_images(cam, nodemap, nodemap_tldevice, i, current_offset_horizontal, current_offset_vertical):
    """
        ...
        while(continue_recording):
            try:               
                ...
                if image_result.IsIncomplete():
                    ...
                    center_x = image_data.shape[1] / 2 + current_offset_horizontal
                    center_y = image_data.shape[0] / 2 + current_offset_vertical
                    
                    # Draws an image on the current figure
                    plt.imshow(image_data, cmap='gray')
                    plt.text(100,100, 'Camera ' + str(device_serial_number), fontfamily='serif', fontsize=10, color='r')
                    plt.scatter(center_x, center_y, s=1, color='red', marker = '+', linewidths=320) #Create rhe red cross in the center of the image
...
def capture_ss(cam, current_offset_horizontal,current_offset_vertical):
    try:
        image_result = cam.GetNextImage()
        if image_result.IsIncomplete():
            print('Image incomplete with image status %d ...' % image_result.GetImageStatus())
        ...
        ax.scatter(center_x, center_y, s=1, color='red', marker='+', linewidths=320)
        ...
        if filename:
            ...
            print('Snapshot saved as %s' % filename)
    except PySpin.SpinnakerException as ex:
        print('Error: %s' % ex)

def run_single_camera(cam, i):
    """
    ...
    try:
        ...
        btn_capture = tk.Button(root, text='Capture', command=lambda:capture_ss)
